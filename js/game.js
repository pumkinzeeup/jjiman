...
function init() {
    ...
    createjs.Sound.registerSounds(
        [{id:"fail", src:"fail.mp3"},{id:"zzeup", src:"jump.mp3"},{id:"zzeup", src:"jump.mp3"},{id:"bgm", src:"bgm.mp3"}], "assets/");
}
...
function handleComplete() {
    ...
    tap.scaleX = 0.2;
    tap.scaleY = 0.2;
    tap.x = w/2 - tap.image.width/2 * 0.2;
    tap.y = h/2 - tap.image.height/2 * 0.2 + 200;
    stage.addChild(tap);
...
function die() {
    ...
    bird.gotoAndPlay("dive");
    createjs.Tween.removeTweens ( bird );
    createjs.Tween.get(bird).wait(0).to({y:bird.y + 200, rotation: 0}, (380)/1.5, createjs.Ease.linear) //rotate back
...
function goShare() {
    ...
    window.open("https://twitter.com/share?url=https://url.kr/j6ld1o/&hashtags=툰툰이를약간이겨라&text=찌만! 툰툰이 [" + countText +  "]마리를 무찔렀찌!!!!");
}
